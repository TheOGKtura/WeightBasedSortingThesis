...
class HX711:
    def __init__(self, dout, pd_sck, gain=128):
        ...
        self.REFERENCE_UNIT = 36
        self.REFERENCE_UNIT_B = 36
    ...
    def read_median(self, times=3):
        """
        Return the median of `times` readings.

        Python 3 fix: avoid float midpoint (len(list)/2).
        Handles both odd and even sample counts.
        """
        if times <= 0:
            raise ValueError("HX711()::read_median(): times must >= 1!!")
        ...
        if length % 2 == 1:
            ...
            return (valueList[mid - 1] + valueList[mid]) / 2.0
